...
class iDPath_Modified(BaseModel):
    def __init__(self, node_num, type_num, adj, emb_dim=16, gcn_layersize=[16, 16, 16], dropout=0.5, deepm="lstm"):
        """
        adding a deepm parameter - means type of deep learning model : lstm, bilstm, rnn, birnn(bidirectional rnn)
        """
        super().__init__()
        self.node_num = node_num
        self.adj = adj
        ...
        self.gcn = GCN(nfeat=gcn_layersize[0], nhid=gcn_layersize[1],
                       nclass=gcn_layersize[2], dropout=dropout)
        ##change whether it is lstm, bilstm, rnn and birnn
        if deepm == "lstm":
            self.lstm = nn.LSTM(input_size=emb_dim*2, hidden_size=emb_dim)
        elif deepm == "bilstm":
            self.lstm = nn.LSTM(input_size=emb_dim*2, hidden_size=emb_dim, bidirectional=True)
        elif deepm == "rnn":
            self.lstm = nn.RNN(input_size=emb_dim*2, hidden_size=emb_dim)
        elif deepm == "birnn":
            self.lstm = nn.RNN(input_size=emb_dim*2, hidden_size=emb_dim, bidirectional=True)
        else:
            self.lstm = nn.LSTM(input_size=emb_dim*2, hidden_size=emb_dim)
